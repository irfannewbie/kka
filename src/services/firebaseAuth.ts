...
export const DEFAULT_ADMIN_USER: User =
  ...
  ADMIN_PROFILES["irfannewbie7@gmail.com"];
...
export const getAuthErrorMessage = (error: any): string => {
  ...
  const currentHost =
    typeof window !== "undefined" ? window.location.hostname : "domain Anda";
  ...
  return error?.message
    ...
    : "Terjadi kendala saat proses autentikasi Firebase.";
};
...
export const getAccessTokenExpiryTimestamp = (): number | null => {
  ...
  return Number.isFinite(expiryTimestamp) ? expiryTimestamp : null;
};
...
export const getSavedAccessToken = (): string | null => {
  ...
  try {
    ...
    const expiry = getAccessTokenExpiryTimestamp();
    if (token && expiry) {
      ...
      if (onAuthSuccess) {
        const savedEmail =
          typeof localStorage !== "undefined"
            ? localStorage.getItem("tugas_siswa_active_admin_email_v3")
            : null;
        const activeProfile =
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "Storage connection closing or restricted. Recovering with inMemoryPersistence...",
          popupErr,
        );
        try {
