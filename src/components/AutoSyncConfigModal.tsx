...
const APPS_SCRIPT_CODE = `function doPost(e) {
  try {
    ...
    var spreadsheetId = data.spreadsheetId || PropertiesService.getScriptProperties().getProperty('TARGET_SPREADSHEET_ID');
    var ss = null;
    ...
    if (!ss) {
      throw new Error('Spreadsheet target tidak ditemukan. Pastikan ID Spreadsheet valid atau script terhubung ke file spreadsheet yang benar.');
    }
    ...
    if (data.action === 'testConnection') {
      return ContentService.createTextOutput(JSON.stringify({
        status: 'success',
        message: 'Koneksi berhasil! Tab sheet "' + sheetName + '" sudah siap menerima data di spreadsheet target.'
...
export const AutoSyncConfigModal: React.FC<AutoSyncConfigModalProps> = ({
    ...
    if (isOpen) {
      ...
      try {
        const raw = localStorage.getItem(
          "tugas_siswa_substitute_submissions_v1",
        );
        if (raw) {
  ...
  const handleTestConnection = async () => {
    ...
    setStatusMessage({
      type: "info",
      text: "Menguji koneksi dan memverifikasi tab sheet di Google Spreadsheet...",
    });
    ...
    if (result.success) {
      setStatusMessage({
        ...
        text:
          result.message ||
          'Koneksi berhasil! Tab sheet "Pengganti KKA 2" telah diverifikasi di Google Spreadsheet Anda.',
      });
    } else {
      setStatusMessage({
        type: "error",
        text:
          result.message ||
          'Gagal menghubungi Web App. Pastikan Deployment diatur ke "Anyone / Siapa Saja".',
      });
    }
  };
  ...
  const handleSyncAllLocal = async () => {
    ...
    setStatusMessage({
      type: "info",
      text: "Sedang mengirim semua data tugas pengganti yang tersimpan di sistem lokal...",
    });

    const res = await syncAllLocalSubstituteTasks(
      spreadsheetId,
      token,
      appsScriptUrl.trim(),
    );
    ...
    if (res.totalSynced > 0) {
      ...
      setStatusMessage({
        ...
        text:
          res.message ||
          "Tidak ada data yang berhasil dikirim. Periksa URL Apps Script atau login Google Anda.",
      });
    }
  };
  ...
  const handleCreateSheetViaToken = async () => {
    ...
    setStatusMessage({
      type: "info",
      text: 'Membuat tab sheet "Pengganti KKA 2" langsung via API Google...',
    });

    const res = await ensureSheetExists(
      token,
      spreadsheetId,
      SUBSTITUTE_TASK_SHEET_NAME,
      SUBSTITUTE_TASK_HEADERS,
      ...
      <div className="bg-[#FAF8F5] border-3 border-[#1a1a1a] shadow-[8px_8px_0px_#1a1a1a] w-full max-w-2xl max-h-[90vh] flex flex-col my-auto">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-300 font-mono-code">
                Agar setiap siswa yang mengirim di /pengganti otomatis masuk ke
                Google Spreadsheet
              </p>
            </div>
          </div>
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-6 text-[#1a1a1a] font-sans">
          ...
          <div className="bg-white border-2 border-[#1a1a1a] p-4 shadow-[3px_3px_0px_#1a1a1a] space-y-3">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              >
                ...
                <span>
                  {isTesting ? "MENGUJI..." : "TES & BUAT TAB SPREADSHEET"}
                </span>
              </button>
            </div>
          </div>
            ...
            <div className="bg-amber-50 border-2 border-amber-600 p-4 shadow-[3px_3px_0px_#d97706] space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-mono-code font-bold text-xs text-amber-950 uppercase flex items-center gap-1.5">
                  ...
                  <span>
                    DATA TUGAS PENGGANTI TERSIMPAN DI LOKAL: {localCount} DATA
                  </span>
                </span>
              </div>
              <p className="text-xs text-amber-900 font-mono-code">
                Ada {localCount} pengumpulan siswa (misal Arjuna Zafif
                Athaillah) yang tercatat di sistem ini. Anda dapat
                menyinkronkannya sekaligus ke Spreadsheet sekarang:
              </p>
              <div className="pt-1">
                <button
                  type="button"
                ...
                >
                  ...
                  <span>
                    {isSyncingAll
                      ? "MENYINKRONKAN..."
                      : `SINKRONKAN ${localCount} DATA KE GOOGLE SPREADSHEET`}
                  </span>
                </button>
              </div>
            </div>
          ...
          <div className="bg-white border-2 border-[#1a1a1a] p-4 shadow-[3px_3px_0px_#1a1a1a] space-y-3">
            ...
            <ol className="list-decimal list-inside space-y-2 text-xs font-mono-code text-slate-700">
              ...
              <li>
                Di menu Spreadsheet, klik <strong>Ekstensi (Extensions)</strong>{" "}
                $\rightarrow$ <strong>Apps Script</strong>.
              </li>
              <li>
                Ganti kode lama dengan kode baru di atas (sudah diatur agar tab
                otomatis dibuat dan diposisikan di{" "}
                <strong>sebelah paling kanan setelah Tugas_Siswa</strong>), lalu
                klik ikon <strong>Simpan (Save / Ctrl+S)</strong>.
              </li>
              <li>
                Klik <strong>Deploy (Terapkan)</strong> $\rightarrow${" "}
                <strong>Manage deployments (Kelola penerapan)</strong>{" "}
                $\rightarrow$ klik ikon pensil <strong>Edit</strong>{" "}
                $\rightarrow$ pada Versi pilih{" "}
                <strong>New version (Versi baru)</strong> $\rightarrow$ klik{" "}
                <strong>Deploy</strong>.
              </li>
              <li>
                Kembali ke aplikasi ini dan klik tombol{" "}
                <strong>"TES & BUAT TAB SPREADSHEET"</strong> di atas. Tab{" "}
                <code>Pengganti KKA 2</code> akan langsung berpindah ke sebelah
                paling kanan.
              </li>
            </ol>
          </div>
        ...
        <div className="bg-[#FAF8F5] px-5 py-3 border-t-2 border-[#1a1a1a] flex items-center justify-between shrink-0 font-mono-code text-xs">
          <div className="flex items-center gap-1.5 text-slate-600">
            ...
            <span>
              Aman & Otomatis: Siswa tidak perlu akun Google atau login
            </span>
          </div>
          <button
            onClick={onClose}
